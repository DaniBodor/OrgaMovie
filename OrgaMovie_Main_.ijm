...
if (nQueuedExp == 0) {
    ...
    Dialog.addCheckbox("Start adding experiment(s) to the queue ?", 0);
...
if (RestartQueueRun) {
    ...
    Dialog.show ExpForRestart = Dialog.getNumber();
}
print("");
if (PrintToTestQueuing) {
    print("");
    ...
    if (Restart) { // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< RESTART BEGIN >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        ...
        if (SameFile == 0) {
            ...
            if (nd2File) {
                ...
                CheckPositionName = 0;
                ReadFileName = 0;
            } // waitForUser("file_"+file+"___________nd2File__"+nd2File);	
        ...
        if (SameFile) {
            ...
            open(file);
            FilePathForInCase = File.directory;
            setLocation(1, 1); //waitForUser(FilePathForInCase);
        }
        ...
        for (i = AmountOfPositions - 1; i >= 0; i--) {
            ...
            if (tiffFile + nd2File == 0) {
                ...
                for (j = PositionChannelAmount[i] - 1; j >= 0; j--) {
                    ...
                    if (tiffFile) {
                        /* I do nothing here, because nd2 is from spinning disk which does not generate transmitted channel */ /*filename=getTitle;*/ }
                    if (nd2File) {
                        ...
                        if (SetMultiplyBeforeDepthcoding) {
                            ...
                            if (SplitZ[i] > 0) {
                                ...
                                while (PlanesDialog) {
                                    ...
                                    } //waitForUser("ALTERNATIVE SETTINGS-LOOP______BottomZ_1[i] : "+BottomZ_1[i]+"_____TopZ_1[i] : "+TopZ_1[i]+"_____BottomZ_2[i] : "+BottomZ_2[i]+"_____TopZ_2[i] : "+TopZ_2[i]);
